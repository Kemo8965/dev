...
export const actions = {
 ...
 //GET ALL TASKS
    ...
    async completeTask({ state, commit }, newTask) {
      ...
      async  updateTaskDetails({ state, commit }) {
        try {
          commit(SET_LOADING, true) 
          //const newCalf = state.selectedCalf.calfBreed


            const newTaskCopy = cloneDeep(state.selectedTask)
          let taskCopyTaskDescription = newTaskCopy.taskDescription
          let taskCopySelectPriority = newTaskCopy.selectPriority
          let taskCopyAssignTask = newTaskCopy.assignTask
          let taskCopyDateAssigned = newTaskCopy.dateAssigned
          let taskCopyDueDate = newTaskCopy.dueDate
       ...
       console.log(taskCopyTaskDescription)
       console.log(taskCopyAssignTask)
       console.log(taskCopyDateAssigned)
       console.log(taskCopyDueDate)
       console.log(taskCopySelectPriority)
       

          const {data: putResponse} = await api.put(`/tasks/updateTask/${newTaskCopy._id}`, {
            newTaskCopy,
            taskDescription:`${taskCopyTaskDescription}`,
            selectPriority:`${taskCopySelectPriority}`,
            assignTask:`${taskCopyAssignTask}`, 
            dateAssigned:`${taskCopyDateAssigned}`,  
            dueDate:`${taskCopyDueDate}`, 
            
          
          })

          commit(UPDATE_SELECTED_TASK, putResponse )
