<template>
  <div>
    ...
    >
       <b-table-column
        v-slot="props"
        field="raisedEarTagID"
        label="Ear Tag ID"
        searchable
        
      >
       <span class="tag  is-light"> {{ props.row.raisedEarTagID }} </span>
...
import TreatmentModal from '~/components/modals/Raised Cattle/Treatment Modal/treatment-modal.vue'
import TreatmentSnapshotModal from '~/components/modals/Raised Cattle/Treatment Modal/treatment-snapshot-modal.vue'
export default {
  ...
  computed: {
    ...
    ...mapGetters('treatmentData', {
        loading: 'loading',
        treatments:'allRaisedCattleTreatments',
      }),
  ...
  async created() {
  // await this.load()
   this.selectRaisedCattleTreatment(this.treatments[0])
  },
  ...
  methods: {
     ...
     ...mapActions('treatmentData', ['addNewTreatment','getAllRaisedCattleTreatments', 'load', 'selectRaisedCattleTreatment']),

     async refresh(){
    //  this.isLoading = true
      await this.getAllRaisedCattleTreatments();
   //   this.isLoading = false
    ...
    captureReceipt(treatment) {
      this.selectRaisedCattleTreatment(treatment)
      setTimeout(() => {
