<template>
  <div>
    ...
    >
      ...
      >
       <span class="tag is-primary is-light"> {{ props.row.purchasedEarTagID }} </span>
...
import ReproductiveModal from '@/components/modals/Purchased Cattle/Reproductive Modal/reproductive-modal.vue'
import ReproductiveSnapshotModal from '@/components/modals/Purchased Cattle/Reproductive Modal/reproductive-snapshot-modal.vue'
export default {
  ...
  computed: {
    ...
    ...mapGetters('reproductiveData', {
        loading:'loading',
        reproductives:'allPurchasedCattleReproductives',
      }),
  ...
  async created() {
  // await this.load()
   this.selectPurchasedCattleReproductive(this.reproductives[0])
  },
  ...
  methods: {
     ...
     ...mapActions('reproductiveData', ['addNewReproductive','getAllPurchasedCattleReproductives', 'load', 'selectPurchasedCattleReproductive']),

     async refresh(){
    //  this.isLoading = true
      await this.getAllPurchasedCattleReproductives();
   //   this.isLoading = false
    ...
    captureReceipt(reproductive) {
      this.selectPurchasedCattleReproductive(reproductive)
      setTimeout(() => {
