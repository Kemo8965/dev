<template>
    <div>
      <div class="card p-5 mr-8">
       ...
       <div class="buttons">
          ...
          <b-tooltip label="Add details of each cow here" type="is-dark">
             <b-button class="mx-2" icon-left="plus-circle" icon-right="cow" type="is-success" @click="addNewCattle">Add New Cattle</b-button>
          </b-tooltip>
      ...
      >
         ...
         <!-- :sticky-header="stickyHeaders" -->
          ...
          <b-table-column
          v-slot="props"
          field="purchasedEarTagID"
          label="Ear Tag ID"
          searchable
          
        >
        
        <span class="tag earTagID ">  {{ props.row.purchasedEarTagID }} </span>
        ...
        <span
            :class="[
              'tag',
              {
                'is-primary is-light ': props.row.cattleProductionStage === 'Heifer'
              },
  
         
              {
                'pink ': props.row.cattleProductionStage === 'Pregnant'
              },
  
              {
                'is-info is-light ': props.row.cattleProductionStage === 'Bull'
              },
  
         
              {
                'is-warning is-light ': props.row.cattleProductionStage === 'Cow'
              },
  
              {
                'is-primary is-light' : props.row.cattleProductionStage === 'Calved & Lactating' ,
              },
             
            ]"
            >  {{ props.row.cattleProductionStage }}</span> 
        </b-table-column> 
       ...
       <b-table-column
        ...
        <span
            :class="[
              ...
              {
                'is-danger': props.row.cattleStatus ===  'Mortality',
              },

              {
                'pink': props.row.cattleStatus ===  'Pregnant',
              },
              {
  ...
  <script>
    ...
    import CattleModal from '~/components/modals/Purchased Cattle/Cattle Modal/cattle-modal.vue'
    import CattleSnapshotModal from '~/components/modals/Purchased Cattle/Cattle Modal/cattle-snapshot-modal.vue'
    import CattleEditModal from '~/components/modals/Purchased Cattle/Cattle Modal/cattle-edit-modal.vue'
  ...
  export default {
    ...
    computed: {
      ...
      tableData() {
        return this.isEmpty ? [] : this.cattleMorts
      },
    },
